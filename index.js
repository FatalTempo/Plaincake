...
const GETvalstats = require("./GET-valstats.js")
const GETrblxstats = require("./GET-rblxstats.js")
const ButtonPages = require('discord-buttons-pages')
...
client.once('ready', () => {
    ...
    commands.create({
      "name": "kanye",
      "description": "Random quote from Kanye himself",
      "options": []
    })
    commands.create({
        name: "rblxstats",
        description: "Returns generic statistics about the user",
        options: [
          {
            type: 3,
            name: "name",
            description: "(REQUIRED) Type in their username NOT their display name. (No @ Symbol)",
            required: true
          }
        ]
      })
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (commandName === "kanye") {
    await interaction.deferReply()
    let arr = ["https://i.imgur.com/yIDzMXn.jpeg", "https://i.imgur.com/aCgcmwn.jpeg","https://i.imgur.com/rSSuFkR.jpeg"]
    async function RetrieveData(link){let GET = await fetch(link);let TXT = await GET.text();let LOL = await JSON.parse(TXT);return LOL}
    let Parsed = await RetrieveData("https://api.kanye.rest")
    let BasicEmbed = new Discord.MessageEmbed()
      .setColor('#EC931C')
      .setTitle(`${Parsed.quote}`)
      .setURL(`${arr[Math.floor(Math.random() * 3)]}`)
    interaction.editReply({embeds: [BasicEmbed]})
  }
  if (commandName === "rblxstats") { 
    let Name = options.getString('name')
    await interaction.deferReply()
    let Parsed = await GETrblxstats(Name)
    let BasicEmbed = new Discord.MessageEmbed()
      .setColor('#EC931C')
      .setTitle(`Showing User Stats for ${Name}`)
      .setURL(`https://www.roblox.com/users/${Parsed.Id}/profile`)
      .setThumbnail(`${Parsed.HeadShot} `)
      .addFields(
        { name: `**Display Name:**`, value: `${Parsed.DisplayName}`,inline: true},
        { name: `**Friend Count:**`, value: `${Parsed.Friends}`,inline: true},
        { name: `**Follower Count:**`, value: `${Parsed.Followers}`,inline: true},
        { name: `**Following Count:**`, value: `${Parsed.Following}`,inline: true},
        { name: '**Currently Wearing:**', value: `${Parsed.Wearing[0]}\n\n**Total Wear Value: ${Parsed.Wearing[1]} <:Robux:1026363599217774672>**`, inline: false}
      )
    interaction.editReply({embeds: [BasicEmbed]})
  }
  if (commandName === "valstats") {
      ...
      const Profile = await GETvalstats(Name, Tag, Gamemode, Region);
      let BasicEmbed = new Discord.MessageEmbed().setColor('#EC931C')
      if (Gamemode === `"Deathmatch"` && Profile.Status === 200){
        let Deathmatch = BasicEmbed
          .setTitle(`Showing ${Gamemode.replaceAll('"', '')} Stats For ${Name}`)
          .setDescription(`*Showing the 5 most recent deathmatch matches*`)
          .addFields(
            ...
            { name: `**⠀Most Kills: ${Profile.Most_Kills}**`, value: `**⠀Score: ${Profile.Score}**`, inline: false},
          )
        ...
        return
      } else if (Profile.Status === 403) {// <:javaScript:813907629989691442> <:Silver_1_Rank:1023431607874494494>
        const ClosedEmbed = BasicEmbed
        ...
        let NextTier = Object.keys(RankIcon)[Object.keys(RankIcon).indexOf(Profile.Rank)+1]
        if (NextTier === "Unrated"){
          NextTier = ''
        }
        NextTier = `\n+${100 - (Profile.Elo.elo % 100)} RR till ${NextTier}`
        if (Profile.Gamemode === `"spikerush"`){Gamemode = "Spike Rush"}
        if (Math.sign(Profile.Elo.mmr_change_to_last_game) === 1){Profile.Elo.mmr_change_to_last_game = "+"+Profile.Elo.mmr_change_to_last_game}
        
        const ServerEmbed = new Discord.MessageEmbed()
        .setColor(`${AgentInfo[Profile.AgentArray[0]].Color}`)
        .setTitle(`Showing ${Gamemode.replaceAll('"', '')} Stats For ${Profile.Name}`) // Screw spike rush
        ...
        .setFooter({ text: `Elo: ${Profile.Elo.elo} (${Profile.Elo.mmr_change_to_last_game})${NextTier}` , iconURL: `${RankIcon[Profile.Rank].Img}` });
              ...
              "components": [
                {
                  ...
                  "disabled": false,
                  "emoji": {
                    "id": `${AgentInfo[Profile.AgentArra  y[0]].Emoji}`,
